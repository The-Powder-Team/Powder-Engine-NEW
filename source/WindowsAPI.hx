...
class WindowsAPI
{
    ...
    RegCreateKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\\\Classes\\\\PowderEngineMod", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, NULL);

    char const *name = "Powder Engine Mod";
    RegSetValueEx(hKey, "", 0, REG_SZ, (BYTE*)name, strlen(name));
    RegSetValueEx(hKey, "FriendlyTypeName", 0, REG_SZ, (BYTE*)name, strlen(name));


    RegCreateKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\\\Classes\\\\PowderEngineMod\\\\shell\\\\open\\\\command", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, NULL);
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\\\Classes\\\\.ycemod", 0, KEY_READ, &ycemodKey) != ERROR_SUCCESS) {
        ...
        char const *name = "PowderEngineMod";

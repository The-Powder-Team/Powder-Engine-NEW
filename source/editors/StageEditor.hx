...
class StageEditor extends MusicBeatState
{
	...
	function addStageLayer():Void
	{
		...
		remove(char2);
		remove(char3);
		var stagenewlayer:FlxSprite = new FlxSprite(Std.parseFloat(eventName3.text),
			Std.parseFloat(eventName4.text)).loadGraphic(Paths.image(eventName2.text));
	...
	function saveStage():Void
	{
		...
		code = code + "\n" + "    ]";
		code = code + "\n" + "}";
		// sys.io.File.saveContent("mods/" + eventName5.text + "/data/stages/" + eventName.text + ".json", code);
		sys.io.File.saveContent("assets/data/stages/" + eventName.text + ".json", code);

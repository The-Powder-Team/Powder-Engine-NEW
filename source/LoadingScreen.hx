...
class LoadingScreen extends FlxState
{
	...
	public override function create()
	{
		...
		Thread.create(function()
		{
			...
			var flashMods = [];
			if (!Settings.engineSettings.data.flashingLightsDoNotShow && Settings.engineSettings.data.flashingLights)
			{ // why show the same warning again
				for (mod => conf in ModSupport.modConfig)
				{
					if (conf.hasFlashingLights == true)
					{
						flashMods.push(mod);
						if (!Settings.engineSettings.data.approvedFlashingLightsMods.contains(mod))
							flashWarning = true;
					}
				}
			}

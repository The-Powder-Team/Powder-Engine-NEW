...
class ExampleModsMenuState extends MusicBeatState
{
	...
	// they're also called weekends idk why i forgor :skull:
	...
	{
		DiscordClient.changePresence("In Weekend Menu", null);
		...
		if (!movedBack && !paused && canSelect)
		{
			...
			if (FlxG.keys.justPressed.G)
			{
				if (Highscore.getWeekendScore(curWeekend) > 0)
					openSubState(new ExampleModsFreeplaySubState(weekendData[curWeekend]));
			}
		}
